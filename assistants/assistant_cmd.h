...
namespace assistant
{
    namespace cmd
    {
        ...
        void run(std::string command, std::function<void(std::string)> receiveOutput) {
            ...
            if (!CreatePipe(&hReadPipe, &hWritePipe, &saAttr, 0)) {
                emitError("командная строка -> Ошибка создания Pipe");
                return;
            }

            if (!SetHandleInformation(hReadPipe, HANDLE_FLAG_INHERIT, 0)) {
                emitError("командная строка -> Ошибка SetHandleInformation");
                return;
            }
            ...
            command = "cmd.exe /c chcp  65001 && " + command;

            if (!CreateProcessA(NULL, command.data(), NULL, NULL, TRUE, CREATE_NO_WINDOW, NULL, NULL, &si, &pi)) {
                std::string error = "командная строка -> Ошибка CreateProcess: " + std::to_string(GetLastError());
                emitError(error);
        ...
        JSValue run(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
            ...
            if (command.empty()) {
                auto return_value = assistant::js::call(ctx, resolve_func, assistant::js::getUndefined(), 0, nullptr);
            ...
            std::thread worker([resolve_func, ctx, receiveOutput, command]() {
                assistant::cmd::run(command, [ctx, receiveOutput](std::string line){
                    wxTheApp->CallAfter([ctx, receiveOutput, line_copy = line]() {
                        if (!JS_IsUndefined(receiveOutput)) {
                            ...
                            JSValue js_argv[1] = { js_line };

                            assistant::js::call(ctx, receiveOutput, assistant::js::getUndefined(), 1, js_argv);
                ...
                wxTheApp->CallAfter([resolve_func, ctx, receiveOutput]() {
                    ...
                    assistant::js::call(ctx, resolve_func, assistant::js::getUndefined(), 0, nullptr);
                    JS_FreeValue(ctx, resolve_func);
